...
public class Brayan extends Enemy {
    ...
    public String attack(GameCharacter enemy) {
        ...
        switch (attack) {
            case 1:
                try {
                    message = desconectedePeda(enemy);
            ...
            case 2:
                try {
                    message = guardameelFilero(enemy);
                } catch (EnemyDeathException e) {
                    enemy.getStats().put(Stats.HP, 0);
                    message = """
    ...
    protected String desconectedePeda(GameCharacter enemy) throws EnemyDeathException {
        int damage = 3;
        int newHP = reduceHP(enemy, damage);
        String enemyName = enemy.getName();
        String message = String.format("""
                ¡%s se desconecta contra ti, %s por %d de daño!
                %s tiene %d HP restantes.
                """, this.name, enemyName, damage, enemyName, newHP);
        return message;
    }

    protected String guardameelFilero(GameCharacter enemy) throws EnemyDeathException {
        int damage = 3;
        int newHP = reduceHP(enemy, damage);
        String enemyName = enemy.getName();
        String message = String.format("""
                ¡%s Te arroja el filero a ti, %s por %d de daño!
                %s tiene %d HP restantes.
                """, this.name, enemyName, damage, enemyName, newHP);
        return message;
    }
